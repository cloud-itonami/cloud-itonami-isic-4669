(ns wastetrade.phase
  "Phase 0->3 staged rollout for the waste/scrap-wholesale actor.

    Phase 0  read-only        -- no writes, still governor-gated.
    Phase 1  assisted-intake  -- waste-order intake allowed, every write
                                 needs human approval.
    Phase 2  assisted-verify  -- adds consent-verification writes,
                                 still approval.
  ...
  real waste/scrap shipment across a border (an autonomous sorting/
  baling/loader robot performs the physical outbound loadout at the
  wholesale yard, or an operator does) and settling a real waste-trade
  ...
  are always a human trading supervisor's call. `wastetrade.governor`'s
  ...
  distinct from the waste-order itself.")

(def read-ops  #{})
(def write-ops #{:order/intake :consent/verify :delivery/dispatch :invoice/settle})
...
(def phases
  ...
  {0 {:label "read-only"        :writes #{}                                              :auto #{}}
   ...
   2 {:label "assisted-verify"  :writes #{:order/intake :consent/verify}                  :auto #{}}
...
(defn verdict->disposition
  "Map a Waste Trading Governor verdict to a base disposition before the
  phase gate."
  [verdict]
