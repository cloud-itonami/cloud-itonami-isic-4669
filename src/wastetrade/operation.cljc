(ns wastetrade.operation
  "OperationActor -- one waste/scrap-wholesale operation = one supervised
  actor run, expressed as a langgraph-clj StateGraph. The advisor
  (WasteTradeAdvisor) is sealed into a single node (:advise); its
  proposal is ALWAYS routed through the Waste Trading Governor
  ...
  One graph run = one waste/scrap-wholesale operation (intake -> advise
  -> govern -> decide -> commit | hold | approval). No unbounded inner
  loop -- each operation is auditable and checkpointed. A waste-order's
  ...
  node when the governor is clean -- see `wastetrade.phase`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [wastetrade.wastetradeadvisor :as wastetradeadvisor]
            [wastetrade.governor :as governor]
...
(defn build
  ...
  `wastetrade.store/Store`).
  opts:
    :advisor      -- a `wastetrade.wastetradeadvisor/Advisor` (default: mock-advisor)
    :checkpointer -- langgraph checkpointer (default: in-mem)"
  [store & [{:keys [advisor checkpointer]
             :or   {advisor      (wastetradeadvisor/mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; WasteTradeAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (wastetradeadvisor/-advise advisor store request)]
            {:proposal p :audit [(wastetradeadvisor/trace request p)]})))

      ;; Waste Trading Governor -- independent censor (separate system than the LLM).
      (g/add-node :govern
